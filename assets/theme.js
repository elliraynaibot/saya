...
var VideoMedia = class extends BaseMedia {
  ...
  connectedCallback() {
    ...
    if (this.getAttribute("type") === "video") {
      if (this.hasAttribute("defer-mobile-playback") && matchesMediaQuery("md-max")) {
        return;
      }
      inView8(this, () => {
  ...
  queueDeferredMobilePlayback({ restart = false } = {}) {
    if (!this.hasAttribute("defer-mobile-playback") || !matchesMediaQuery("md-max")) {
      this.play({ restart });
      return;
    }
    if (this.hasAttribute("mobile-playback-ready")) {
      this.play({ restart });
      return;
    }
    if (this.hasAttribute("mobile-playback-queued")) {
      return;
    }
    this.setAttribute("mobile-playback-queued", "");
    const startPlayback = () => {
      if (this.hasAttribute("mobile-playback-ready")) {
        return;
      }
      this.setAttribute("mobile-playback-ready", "");
      this.querySelector("video")?.setAttribute("preload", "auto");
      this.play({ restart });
    };
    const mobileAutoplayDelay = 3e3;
    const scheduleMobileAutoplay = () => {
      if (this._abortController.signal.aborted) {
        return;
      }
      const autoplayTimer = window.setTimeout(startPlayback, mobileAutoplayDelay);
      this._abortController.signal.addEventListener("abort", () => window.clearTimeout(autoplayTimer), { once: true });
    };
    const listenerOptions = { once: true, passive: true, signal: this._abortController.signal };
    window.addEventListener("pointerdown", startPlayback, listenerOptions);
    window.addEventListener("touchstart", startPlayback, listenerOptions);
    window.addEventListener("scroll", startPlayback, listenerOptions);
    window.addEventListener("keydown", startPlayback, { once: true, signal: this._abortController.signal });
    if (document.readyState === "complete") {
      scheduleMobileAutoplay();
    } else {
      window.addEventListener("load", scheduleMobileAutoplay, { once: true, signal: this._abortController.signal });
    }
  }
  _playerTarget() {
...
// js/sections/deferred-product-reviews.js
var DeferredProductReviews = class extends HTMLElement {
  connectedCallback() {
    this._contentElement = this.querySelector("[data-reviews-content]");
    this._templateElement = this.querySelector("template");
    if (!this._contentElement || !this._templateElement) {
      return;
    }
    this._isLoaded = false;
    this._summaryClickListener = this._summaryClickListener || this.onSummaryClick.bind(this);
    this._intersectionObserver = this._intersectionObserver || new IntersectionObserver(this.onIntersection.bind(this), { rootMargin: "300px 0px" });
    this._intersectionObserver.observe(this);
    document.addEventListener("click", this._summaryClickListener, true);
    if (window.location.hash === `#${this.id}`) {
      this.loadReviews().then(() => this.scrollIntoView({ block: "start" }));
    }
  }
  disconnectedCallback() {
    this._intersectionObserver?.disconnect();
    document.removeEventListener("click", this._summaryClickListener, true);
    if (this._judgeMeInitTimer) {
      clearInterval(this._judgeMeInitTimer);
    }
  }
  ...
  async onSummaryClick(event) {
    const trigger = event.target instanceof Element ? event.target.closest(`a[href="#${this.id}"]`) : null;
    if (!trigger || this._isLoaded) {
      return;
    }
    event.preventDefault();
    ...
    this.scrollIntoView({ behavior: window.matchMedia("(prefers-reduced-motion: reduce)").matches ? "auto" : "smooth", block: "start" });
  }
  async loadReviews() {
    if (this._isLoaded || !this._contentElement || !this._templateElement) {
      return;
    }
    this._isLoaded = true;
    this.setAttribute("aria-busy", "true");
    this._contentElement.append(this._templateElement.content.cloneNode(true));
    this._contentElement.hidden = false;
  ...
  initializeJudgeMe() {
    const tryInitialize = () => {
      try {
        if (typeof window.jdgm?.initWidgets === "function") {
        ...
        if (typeof window.jdgm?.init === "function") {
          window.jdgm.init();
          return true;
        }
      } catch (error) {
    ...
    let attempts = 0;
    this._judgeMeInitTimer = window.setInterval(() => {
      attempts += 1;
      if (tryInitialize() || attempts >= 20) {
        clearInterval(this._judgeMeInitTimer);
        this._judgeMeInitTimer = null;
      }
    }, 250);
  }
};
if (!window.customElements.get("deferred-product-reviews")) {
  window.customElements.define("deferred-product-reviews", DeferredProductReviews);
}
...
onSlideChange_fn = function(event) {
  ...
  if (event.detail.cell.getAttribute("data-slide-type") === "video") {
    event.detail.cell.querySelectorAll("video-media").forEach((video) => {
      if (video.hasAttribute("defer-mobile-playback") && matchesMediaQuery("md-max")) {
        video.queueDeferredMobilePlayback({ restart: true });
      } else {
        video.play({ restart: true });
      }
    });
  }
};
